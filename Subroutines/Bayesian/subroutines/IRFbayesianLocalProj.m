...
%Sigma~IW(S,a); vecB|Sigma~N(B,V); V=kron(Sigma,Omega); vecB=[n*(1+n*nLv)x1]
 ...
 %set prior residual variance (Sigma) using univariate AR(1) residual
sigmaj=NaN(n,1);
for j=1:n
...
if contains(modelSpec.priorType,'DSGE')==1 %so VAR (h=1) is also affected by the model based prior
      
    B_init(1+n,:)=modelSpec.irfDSGE(2,:);
...
%load results
lpirfLowerBound(:,1:2) =StructuralIRFsCollect(:,1:2,round(lBound*nRetainedDraws));
lpirfUpperBound(:,1:2) =StructuralIRFsCollect(:,1:2,round(uBound*nRetainedDraws));
...
for h=2:nH
    ...
    iXhprojSet= (XhprojSet'*XhprojSet)\eye(n*nL+1);
    ...
    switch priorType
        ...
        case 'DSGE' 
            ...
            % centred on IRF from DSGE 
            Bh_init=zeros(numel(vecB)/n,n);
            Bh_init(2:n+1,:)=diag(1.*isRandomWalk);
            Bh_init(1+n,:)=modelSpec.irfDSGE(h+1,:);
    ...
    for jj=1:n
        ...
        sigmaBj = iXhprojSet*SigmaU*iXhprojSet; %HAC error (T*)covariance estimator;
        sigmaB = sigmaBj(2:n+1,2:n+1);  %remove only first lags
        
        Omega2=(shockS'*Bzero)*sigmaB*(shockS'*Bzero)'; 
        Omega(jj,h)=Omega2.^0.5;

        lpirfUpperBound(jj,h+1)=irfs(jj,h+1)+Omega(jj,h)*sLevel;
        lpirfLowerBound(jj,h+1)=irfs(jj,h+1)-Omega(jj,h)*sLevel;
    end
...
res.st_dev       = (res.irfs_u - res.irfs)./sLevel;
